...
router.beforeEach((to)=>{const auth=useAuthStore(); auth.loadFromStorage(); if(auth.isLoggedIn && ['/login','/register'].includes(to.path)) return '/'; if(to.meta.requiresAdmin){ if(!auth.isLoggedIn) return {path:'/login',query:{redirect:to.fullPath}}; if(!auth.isAdmin) return '/no-access'} if(to.meta.requiresAuth && !auth.isLoggedIn) return {path:'/login',query:{redirect:to.fullPath}}; return true})
