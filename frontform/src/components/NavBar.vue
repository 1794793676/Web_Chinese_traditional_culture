<template>
  <header class="site-header">
    <div class="site-header__inner">
      ...
      <button class="icon-btn nav-toggle" type="button" @click="menuOpen = !menuOpen">☰</button>
...
const goFirstCategory = async () => {
  menuOpen.value = false
  let slug = 'thought'

  try {
    const list = await getCategories()
    slug = list?.[0]?.slug || slug
  } catch {
    // ignore
  }

  if (!auth.isLoggedIn) {
    return router.push({ path: '/login', query: { redirect: `/category/${slug}` } })
  }
